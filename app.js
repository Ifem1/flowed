...
const state = { wallet: '', chainId: 0, flows: [], accounting: null, selected: null };
...
async function refresh(){
  if(!/^0x[a-fA-F0-9]{40}$/.test(CONTRACT)){const b=$('#deployment-banner');b.hidden=false;b.innerHTML='<strong>Deployment pending.</strong><p>The application is wired for Studionet 61999, but no canonical contract address is configured. No fake data is shown.</p>';state.flows=[];renderAll();return;}
  try{const [countRaw,accounting]=await Promise.all([read('get_flow_count'),read('get_accounting')]);const count=BigInt(countRaw),flows=[];for(let id=1n;id<=count;id+=1n){const [flowRaw,activeRaw]=await Promise.all([read('get_flow',[id]),read('get_active_step',[id])]);const flow=safeJson(flowRaw);flow.id=id.toString();flow.active_step_view=activeRaw?safeJson(activeRaw):null;flows.push(flow);}state.flows=flows;state.accounting=accounting;renderAll();}catch(error){toast(error.message||String(error),true);$('#flow-grid').innerHTML=`<div class="panel empty">Live read failed: ${escapeHtml(error.message||String(error))}</div>`;}
}
function card(flow){const total=flow.steps?.length||0,active=Number(flow.active||0),progress=flow.state==='COMPLETED'?total:Math.min(active,total);return `<button class="flow-card" data-flow-id="${flow.id}"><div class="flow-card-top"><span class="status ${String(flow.state).toLowerCase()}">${escapeHtml(flow.state)}</span><span>#${flow.id}</span></div><h3>${escapeHtml(flow.title)}</h3><p>${short(flow.payer)} → ${short(flow.recipient)}</p><div class="flow-money"><span>Funded <b>${formatGen(flow.escrow)} GEN</b></span><span>Remaining <b>${formatGen(flow.remaining)} GEN</b></span></div><div class="progress"><i style="width:${total?(progress/total)*100:0}%"></i></div><small>${progress}/${total} released · active step ${['ACTIVE','PROVISIONAL','CONTESTED'].includes(flow.state)?active+1:'—'}</small></button>`;}
function listHtml(flows){return flows.length?flows.map(card).join(''):'<div class="empty">No matching flows.</div>';}
function renderStats(){const a=state.accounting;if(!a)return;$('#global-stats').innerHTML=`<div class="stat-card"><span>FUNDED</span><strong>${formatGen(a.funded)} <small>GEN</small></strong></div><div class="stat-card"><span>RELEASED</span><strong>${formatGen(a.released)} <small>GEN</small></strong></div><div class="stat-card"><span>REMAINING</span><strong>${formatGen(a.remaining)} <small>GEN</small></strong></div><div class="stat-card"><span>FLOWS</span><strong>${state.flows.length}</strong></div>`;}
function renderDashboard(){const w=state.wallet.toLowerCase();if(!w){$('#payer-flows').innerHTML='<div class="empty">Connect an injected wallet to filter participant flows.</div>';$('#recipient-flows').innerHTML='<div class="empty">Public browsing remains available without a wallet.</div>';return;}$('#payer-flows').innerHTML=listHtml(state.flows.filter(f=>String(f.payer).toLowerCase()===w));$('#recipient-flows').innerHTML=listHtml(state.flows.filter(f=>String(f.recipient).toLowerCase()===w));}
function availableActions(flow){const w=state.wallet.toLowerCase(),payer=String(flow.payer).toLowerCase(),recipient=String(flow.recipient).toLowerCase(),now=Math.floor(Date.now()/1000),a=[];if(flow.state==='OFFERED'){if(w===recipient&&now<=Number(flow.accept_by))a.push(['accept_flow','Accept Flow']);if(w===recipient)a.push(['decline_flow','Decline']);if(w===payer)a.push(['withdraw_offer','Withdraw offer']);if(now>Number(flow.accept_by))a.push(['expire_unaccepted_flow','Expire offer']);}if(flow.state==='ACTIVE'){if(w===recipient)a.push(['review_active_step','Review active step'],['abandon_flow','Abandon Flow']);const s=flow.steps?.[Number(flow.active)];if(s?.deadline&&now>Number(s.deadline))a.push(['expire_active_flow','Expire active Flow']);}if(flow.state==='PROVISIONAL'){if(w===payer&&now<Number(flow.contest_deadline))a.push(['contest_active_step',`Contest · ${formatGen(BigInt(flow.steps[Number(flow.active)].amount_wei)/20n)} GEN bond`]);if(now>=Number(flow.contest_deadline))a.push(['finalize_active_step','Finalize step']);}if(flow.state==='CONTESTED'){a.push(['resolve_contest','Resolve contest']);if(now>=Number(flow.contest_opened_at)+86400)a.push(['finalize_stalled_contest','Finalize stalled contest']);}return a;}
...
function resetStepIndexes(){$$('.step-editor').forEach((n,i)=>{n.dataset.step=i;n.querySelector('h3').textContent=`Step ${i+1}`;});updateCreateTotal();}
function addStep(){const e=$('#step-editor');if(e.children.length>=8)return toast('Flowed supports at most 8 ordered steps.',true);e.insertAdjacentHTML('beforeend',stepTemplate(e.children.length));bindStepInputs();}
function bindStepInputs(){$$('.step-amount').forEach(i=>{i.oninput=updateCreateTotal;});$$('.remove-step').forEach(b=>{b.onclick=()=>{if($('#step-editor').children.length<=2)return toast('Flowed requires at least 2 steps.',true);b.closest('.step-editor').remove();resetStepIndexes();};});}
function updateCreateTotal(){let total=0n,valid=true;$$('.step-amount').forEach(i=>{if(!i.value)return;try{total+=parseGen(i.value);}catch{valid=false;}});$('#create-total').textContent=valid?`${formatGen(total)} GEN`:'Invalid amount';}
